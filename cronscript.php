<?php
    ...
    if (check_enabled()) {
        ...
        if (empty($examscmidscist) == false) {
            $notrendered = $DB->execute("update {plagiarism_tomagrade}  set finishrender = 1 where id in (  select id from ".
            ...
            $data = $DB->get_records_sql("select * from {plagiarism_tomagrade_config} ".
